...
export default function DoctorList() {
  ...
  const doctorList = useSelector((state) => state.User.doctorList);
  ...
  useEffect(() => {
    setPageCount(doctorList.totalPages);
  }, [doctorList]);
  ...
  useEffect(() => {
    console.log("===================", "received user", doctorList);
  }, [doctorList]);

  const getDoctorList = () => {
    UserAction.startLoading();
    dispatch(
      UserAction.getDoctors({
  ...
  const updateUserAccess = (index, access) => {
    // setIsLoading(true)
    console.log(`email id is : ${doctorList?.docs[index]?.email}`);
    // let userMail = { email: userList?.docs[index]?.email };
    let updatedAccess = access === "grant" ? "revoke" : "grant";
    dispatch(
      UserAction.updateUserAccess({
        id: doctorList?.docs[index]?.id,
        body: { access: updatedAccess },
      })
    ).then(() => {
      console.log(`User access changed successfully`);
      getDoctorList();
    });
  };

  let view = doctorList?.docs?.map((user, i) => (
    <tr>
      <td> {user.id}</td>
      <td> {user.email}</td>
      <td> {user.access}</td>
      ...
      <td>
        <Button
          ...
          color="primary"
          onClick={() => {
            updateUserAccess(i, user.access);
          }}
          type="button"
        >
          {user.access === "grant" ? "Revoke" : "Grant"}
        </Button>
      </td>
    </tr>
  ));
                    ...
                    {doctorList?.docs?.length ? (
                      <>
                        ...
                        >
                          ...
                          <tbody>
                            {view}
                            {doctorList.docs.map((item, index) => {
                              <tr key={index}>
                                <td>{item.id}</td>
                                <td>{item.Email}</td>
                                <td>{item.Access}</td>
