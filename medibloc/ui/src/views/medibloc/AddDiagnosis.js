...
const AddDiagnosis = (props) => {
  ...
  const diagnosis = useSelector((state) => state.Agreement.diagnosis);
  ...
  const getDiagnosis = () => {
    if (PrescriptionDetails?.id) {
      dispatch(AgreementAction.getDiagnosis(PrescriptionDetails?.id))
        .then(() => {
          addToast(`Diagnosis fetched-------------- successfully`, {
            appearance: "success",
            autoDismiss: true,
          });
        })
  ...
  const validateDiagnosis = () => {
    ...
    if (comment == "") {
      addToast(`Please add your comment `, {
        appearance: "error",
        autoDismiss: true,
      });
      isInvalid = true;
  ...
  const requestTheAdd = () => {
    ...
    dispatch(
      AgreementAction.createDiagnosis({
