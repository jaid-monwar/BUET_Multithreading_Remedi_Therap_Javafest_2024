...
export default function Register() {
  ...
  const [isPatientDepartment, setIsPatientDepartment] = useState(true);
  const [isDoctorDepartment, setIsDoctorDepartment] = useState(false);
  const [isPharmacistDepartment, setIsPharmacistDepartment] = useState(false);
  ...
  const inputChangeHandler = (value, fieldName) => {
    switch (fieldName) {
      ...
      case "password":
        setPassword(value);
        break;
      case "isDoctorDepartment":
        if (!isDoctorDepartment) {
          setIsDoctorDepartment(!isDoctorDepartment);
          //console.log(`Legal Dept value is : ${isLegalDepartment}`);
          if (isPatientDepartment) {
            setIsPatientDepartment(false);
          }
          if (isPharmacistDepartment) {
            setIsPharmacistDepartment(false);
          }
        }
        break;
      case "isPatientDepartment":
        if (!isPatientDepartment) {
          setIsPatientDepartment(!isPatientDepartment);
          //console.log(`Legal Dept value is : ${isLegalDepartment}`);
          if (isDoctorDepartment) {
            setIsDoctorDepartment(false);
          }
          if (isPharmacistDepartment) {
            setIsPharmacistDepartment(false);
          }
        ...
        break;
      case "isPharmacistDepartment":
        if (!isPharmacistDepartment) {
          setIsPharmacistDepartment(!isPharmacistDepartment);
          //console.log(`Legal Dept value is : ${isLegalDepartment}`);
          if (isDoctorDepartment) {
            setIsDoctorDepartment(false);
          }
  ...
  const createAccount = () => {
    ...
    let userData = {
      ...
      department: isPatientDepartment ? "patient" : isDoctorDepartment ? "doctor" : "pharmacist",
    };
            ...
            <Form role="form">
              ...
              <FormGroup>
                ...
                <div>
                  <CustomInput
                    type="checkbox"
                    checked={isDoctorDepartment}
                    onChange={(e) => {
                      inputChangeHandler(e.target.value, "isDoctorDepartment");
                    }}
                  ...
                  <CustomInput
                    type="checkbox"
                    checked={isPharmacistDepartment}
                    onChange={(e) => {
                      inputChangeHandler(
                        e.target.value,
                        "isPharmacistDepartment"
                      );
                    }}
                    id="exampleCustomInline3"
                    label="Pharmacist Department"
                    inline
                  />
                </div>
              </FormGroup>
