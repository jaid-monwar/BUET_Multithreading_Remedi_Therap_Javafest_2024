...
const ContractView = (props) => {
  ...
  const getPersonalInfo = () => {
    if (contractDetails?.id) {
      dispatch(AgreementAction.getPersonalInfo(contractDetails?.id))
        .then(() => {
          addToast(`personal info fetched-------------- successfully`, {
            appearance: 'success',
            autoDismiss: true,
          });
        })
      ...
      {contractDetails ? (
        <Modal isOpen={modal} toggle={toggle} className={className} size="lg">
          <ModalHeader className="border-0" toggle={toggle} bsSize='lg'>
            <h3 className="mb-0">Prescription Details</h3> </ModalHeader>
