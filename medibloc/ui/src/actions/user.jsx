...
export const START_GET_DOCTORS = `START_GET_DOCTORS`;
export const END_GET_DOCTORS = `END_GET_DOCTORS`;

export const START_GET_PHARMACISTS = `START_GET_PHARMACISTS`;
export const END_GET_PHARMACISTS = `END_GET_PHARMACISTS`;
...
export const START_USERS_ACCESS_UPDATE = `START_USERS_ACCESS_UPDATE`;
export const END_USERS_ACCESS_UPDATE = `END_USERS_ACCESS_UPDATE`;
...
export function getUsers(data) {
  return (dispatch) => {
    ...
    let config = {
      method: "get",
      url: `${routes.getUsers}?${queryParams}`,
      headers: s.headers,
    };
...
export function getDoctors(data) {
  return (dispatch) => {
    ...
    console.log("printing query params", queryParams);
    dispatch(createAction(START_GET_DOCTORS));
    let s = headers();

    // let header = headers()
    let config = {
      method: "get",
      url: `${routes.getDoctors}?${queryParams}`,
      headers: s.headers,
    };
    ...
    return axios(config)
      .then((response) => {
        ...
        dispatch(createAction(END_GET_DOCTORS, response.data.payload));
        return response.data.data;
      })
      .catch((err) => {
        dispatch(createAction(END_GET_DOCTORS));
        console.log("Error occurred", null, err);
        return null;
      });
  };
}
export function getPharmacists(data) {
  return (dispatch) => {
    let queryParams;
    console.log("==========paginatiion------------===========", data);
    if (data) {
    ...
    console.log("printing query params", queryParams);
    dispatch(createAction(START_GET_PHARMACISTS));
    let s = headers();

    // let header = headers()
    let config = {
      method: "get",
      url: `${routes.getPharmacists}?${queryParams}`,
      headers: s.headers,
    };
    ...
    return axios(config)
      .then((response) => {
        ...
        dispatch(createAction(END_GET_PHARMACISTS, response.data.payload));
        return response.data.data;
      })
      .catch((err) => {
        dispatch(createAction(END_GET_PHARMACISTS));
        console.log("Error occurred", null, err);
        return null;
      });
  };
...
export function updateUserStatus(data) {
  return (dispatch) => {
    ...
    let h = headers();
    return axios
      .put(`${routes.updateUserStatus}${data.id}`, data.body, h)
      .then((resp) => {
        console.log(
          "action : getting response ======================resp==================",
          resp
        );
        if (resp && resp.data) {
...
export function updateUserAccess(data) {
  return (dispatch) => {
    let url = "http://localhost:3004/users";
    dispatch(createAction(START_USERS_ACCESS_UPDATE));
    dispatch(createAction(START_LOADING));
    let h = headers();
    return axios
      .put(`${routes.updateUserAccess}${data.id}`, data.body, h)
      .then((resp) => {
        console.log(
          "action : getting response ======================resp==================",
          resp
      ...
      .finally(() => {
        dispatch(createAction(END_LOADING));
        dispatch(createAction(END_USERS_ACCESS_UPDATE));
      });
  };
}
...
export function registerUser(data) {
  return (dispatch) => {
    ...
    return axios
      .post(routes.registerUser, data, headers())
      .then((resp) => {
        dispatch(createAction(END_LOADING));
        console.log(
          "action : getting response ======================resp==================",
